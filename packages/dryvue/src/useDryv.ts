import type {
  ...
  DryvOptions,
  DryvValidatable,
  DryvValidationResult, DryvValidationRuleSet,
...
export function useDryv<TModel extends object>(
  ...
  switch (typeof ruleSet) {
    case "undefined":
      throw new Error(`The ruleSet parameter must be either a valid rule set name of a valid rule set.`);

    case "string": {
      const ruleSetName = ruleSet;
      ruleSet = dryvRuleSet<TModel>(ruleSetName);
      if (!ruleSet) {
