...
DryvJS is designed as the **client-side runtime** for validation rules generated by [**Dryv**](https://github.com/mhusseini/dryv) (a .NET library that translates C# validation expressions to JavaScript). It can also be used **standalone** with rule sets defined directly in TypeScript. For Vue 3 integration, see [**Dryvue**](https://github.com/mhusseini/dryvjs/tree/develop/packages/dryvue).
...
- [Usage](#usage)
  ...
  - [Grouped Validation Messages](#grouped-validation-messages)
  - [Disablers](#disablers)
  - [Parameters](#parameters)
  - [Warnings vs. Errors](#warnings-vs-errors)
  - [Dirty Tracking](#dirty-tracking)
  - [Server Validation Results](#server-validation-results)
  - [Validation Triggers](#validation-triggers)
  - [Customizing Options](#customizing-options)
  - [Rule Set Resolvers](#rule-set-resolvers)
  - [Merging Options](#merging-options)
  - [Utility Functions](#utility-functions)
- [Validator Hierarchy](#validator-hierarchy)
- [API Reference](#api-reference)
  - [DryvValidationSession](#dryvvalidationsession)
  - [DryvValidator (base)](#dryvvalidator-base)
- [Using with Dryv (C#/.NET)](#using-with-dryv-cnet)
  - [The Generated Output Format](#the-generated-output-format)
  - [The `$ctx` Parameter](#the-ctx-parameter)
  - [Example: C# Rule → DryvJS Consumption](#example-c-rule--dryvjs-consumption)
  - [Configuring `callServer` for Your Backend](#configuring-callserver-for-your-backend)
- [License](#license)
...
DryvJS is the intended client-side runtime for [**Dryv**](https://github.com/mhusseini/dryv). Dryv translates C# validation expressions into JavaScript objects that conform to the `DryvValidationRuleSet` interface.

### The Generated Output Format

Dryv produces a JavaScript object with this structure:
...
```javascript
{
  ...
  validators: {
    "propertyName": [{
      group: "optionalGroup",
      async: true,                       // only when true
      related: ["otherProp"],            // related property paths
      annotations: { required: true },   // metadata from annotators
...
This is identical to `DryvValidationRuleSet` — pass it directly to `DryvValidationSession` or to Dryvue's `useDryv()`.

### The `$ctx` Parameter

In Dryv-generated rules, the second argument (`$ctx`) is the `DryvValidationSession`. It exposes:

- `$ctx.parameter("name")` → `session.parameter(key)` — access named parameters.
- `$ctx.dryv.callServer(url, method, data)` → calls the configured server endpoint (async rules).
- `$ctx.dryv.handleResult(ctx, model, prop, group, result)` → processes the server response.
- `$ctx.dryv.parseDate(value, culture, format)` → parses date strings (from `DateTime` comparisons).
- `$ctx.dryv.format(value, type, format)` → formats values (from `.ToString(format)` translations).

### Example: C# Rule → DryvJS Consumption
...
public static readonly DryvRules Rules = DryvRules
    ...
    .Rule(
        ...
        m => string.IsNullOrWhiteSpace(m.Email) && string.IsNullOrWhiteSpace(m.Phone)
            ? DryvValidationResult.Error("Please provide either email or phone.")
            : DryvValidationResult.Success,
        new DryvRuleSettings("contact"));
```

**Generated JavaScript (consumed by DryvJS):**
...
```javascript
{
  validators: {
    email: [{
      ...
      group: "contact",
      validate: function ($m, $ctx) {
        return !/\S/.test($m.email || "") && !/\S/.test($m.phone || "")
          ? { type: "error", text: "Please provide either email or phone.", group: "contact" }
          : null;
      }
    }],
    phone: [{
...
// ruleSet is the Dryv-generated object (loaded from API or imported from generated file)
...
### Configuring `callServer` for Your Backend

If your Dryv dynamic controllers require authentication or custom headers:
...
For the full integration guide (build-time code generation, runtime loading, Razor integration), see the [root README](../../README.md#using-with-dryv-c).
