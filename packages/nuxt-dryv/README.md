...
nuxt-dryv integrates the [**DryvJS**](../dryvjs/README.md) validation engine into [Nuxt 3](https://nuxt.com) via [**Dryvue**](../dryvue/README.md). It handles plugin registration, Nuxt-native server communication with `$fetch`, and an extensible result handler pipeline — all with full SSR support.
...
- **Automatic Vue plugin registration** — Installs the `@softwareproduction/dryvue` plugin with a Vue `reactive()` wrapper
...
- **Auto-imported composables** — `useDryv`, `addResultHandler`, and `removeResultHandler` are available without explicit imports
...
### Configuring `loadParameters` and Other Hooks

The module sets up `callServer` (using `$fetch`), `reactiveWrapper`, and `handleResult` automatically. The only hook you *must* implement is `loadParameters` — and only if your rule sets use parameters.

Use the `dryv:options` hook in a plugin to provide it via `options.setup` — a factory function that returns a `Partial<DryvOptions>`. Because `setup` is called lazily at validation time (within a component's lifecycle), Nuxt composables like `useState` or custom GraphQL clients are safely available:
...
The `dryv:options` hook fires after the module has configured the base options but before the plugin is installed, so you can override or extend any property on the `DryvOptions` object. Any hook returned from `setup` takes precedence over the module's defaults.
